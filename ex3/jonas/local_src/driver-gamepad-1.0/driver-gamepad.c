...
// iowrite is needed (ldd3 chapter 9, p. 250)
void gpio_init()
{
...
static int __init template_init(void)
{
    ...
    {
        printk(KERN_ERR "Error allocating memory region GPIO_IF\n");
        return -1;
    }
    ...
    err  = request_irq(17, (irq_handler_t) short_probing, 0, DEVICE_NAME, &template_cdev;
    err += request_irq(18, (irq_handler_t) short_probing, 0, DEVICE_NAME, &template_cdev;
    // error if one of the two returns a number != 0
    if (err)
    {
        printk(KERN_ERR "Error in initiating inerrupt handler");
        return -1;
    }
...
irqreturn_t short_probing(int irq, void *dev_id, struct pt_regs *regs)
        {
            ...
            irq_value = ioread8(GPIO_IF) & ~ioread8(GPIO_PC_DIN);
            // clear interrupt
